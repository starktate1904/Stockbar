        ...
        if csv_file and csv_file.name.endswith('.csv'):
            ...
            for row in reader:
                try:
                    ...
                    product.save()
                except KeyError as e:  # Handle missing keys in the CSV
                    message = f"Error processing row: {row}. Missing column: {e.args[0]}"
                    messages.error(request, message)  # Display error message
                except ValueError as e:  # Handle invalid data types
                    message = f"Error processing row: {row}. Invalid data: {e}"
                    messages.error(request, message)  # Display error message
                except ValidationError as e:  # Handle validation errors
                    message = f"Error saving product: {e.messages}"
                    messages.error(request, message)  # Display validation error message
                except Exception as e:  # Catch any other exceptions
                    message = f"Unexpected error processing row: {row}. Error: {str(e)}"
                    messages.error(request, message)  # Display unexpected error message
...
def product_create(request):
    ...
    if request.method == 'POST':
        ...
        # Validate input
        if not name or not location or not price or not quantity or not branch_id or not category_id:
        ...
        Product.objects.create(name=name,  location=location, price=price, branch=branch, quantity=quantity, category=category)
